...
<script type="text/javascript">
	...
	var getContractResolveUrl = "<%=request.getContextPath()%>/contractresolve/getAll";//默认无分页，分页需后面拼接page
	var saveBatchContractResolveUrl = "<%=request.getContextPath()%>/contractresolve/savebatch";
...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/contract/contractresolve.js?v=<%=Math.random()%>"></script>
	...
	<div class="easyui-layout" data-options="fit:true" style="min-width: 900px;">
		<div data-options="region:'north',border:false" style="width: 100%; height: 60px; border-bottom: 1px dashed #ccc; margin-bottom: 15px;">
			<div class="easyui-panel my_button1 my_combobox" data-options="fit:true,border:false" style="padding: 15px 5px 0px 5px;">
				<a id="_add" href="javascript:void(0);" class="easyui-menubutton my_add" data-options="iconCls:'icon-help'" data-options="disabled:true">拆分</a>
			...
			<div class="my_combobox" data-options="region:'center'" style="padding: 10px">
				<div>
					...
					<input type="hidden" id="contractId" value="0" />
						<table style="margin-top: 10px;" id="table_resolve">
							<tr>
								<td>1</td>
								...
								<td style="padding-left: 20px;"><a id="add_resolve" title="添加" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'"></a></td>
							</tr>
						</table>
					</form>
				</div>
